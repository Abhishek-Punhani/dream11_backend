...
match_no = int(input())
if match_no == 1:
    df = pd.read_csv(os.path.join(script_dir, "data", "full_dataset.csv"))
    df=df[(df['team']=='Royal Challengers Bangalore') | (df['team']=='Delhi Capitals')]
    df=df[df['start_date']=='2024-03-17']
    df.to_csv(os.path.join(script_dir, "data", "file_6.csv"))
elif match_no == 2:
    df = pd.read_csv(os.path.join(script_dir, "data", "file_2.csv"))
else:
    df = pd.read_csv(os.path.join(script_dir, "data", "file_3.csv"))
